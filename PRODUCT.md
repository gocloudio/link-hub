...
- 名称、链接和至少一个分类必填，描述可空；概览不展示描述或详细说明。
...
- 新增、编辑卡片使用右侧抽屉，保存成功后更新当前页面；Markdown 仅在编辑抽屉查看和维护。
- 普通用户可维护自己的私有卡片，指定已登录过本站的成员只读分享；管理员可以查看和维护全部公开与私有卡片。
...
- 所有用户登录后才能查看；内部公开卡片对登录用户可见，普通用户对公开卡片及收到的分享只读。每个人的收藏、排序独立存储在 PostgreSQL。
